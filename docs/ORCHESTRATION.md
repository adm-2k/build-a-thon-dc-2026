...
discontinued 2027 (addendum §1). The SPEC §7 var list is extended by three
...
```bash
# P0-1 Gemini — see §1.3 while verification is pending
curl -s "https://generativelanguage.googleapis.com/v1beta/models?key=$GOOGLE_GENERATIVE_AI_API_KEY" | head -c 300

# P0-2 Supabase (after DDL): insert + read back one events row
curl -s "$SUPABASE_URL/rest/v1/events" -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" \
  -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY" -H "Content-Type: application/json" \
  -d '{"instrument":"01","verb":"SMOKE"}' && \
curl -s "$SUPABASE_URL/rest/v1/events?select=*&limit=1" \
  -H "apikey: $SUPABASE_SERVICE_ROLE_KEY" -H "Authorization: Bearer $SUPABASE_SERVICE_ROLE_KEY"

# P0-3 Vercel: production URL renders after first deploy; then confirm the
# preview-protection decision by opening a branch preview in a logged-out window

# P0-4 Tavily
curl -s https://api.tavily.com/search -H "Authorization: Bearer $SEARCH_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query":"nuclear power decarbonization","max_results":3}' | head -c 400

# P0-5 Hugging Face (also the DATA-CAVEATS warm-up call — time it; <3s keeps the pin)
time curl -s https://router.huggingface.co/v1/chat/completions \
...
| Never | `cd` into another worktree, edit outside the ownership map, or run `git push` to `main` |

### 2.3 Ownership map (the collision contract)

| Lane | Branch | Owns (globs) | Shared-file exceptions |
|---|---|---|---|
| A — Engine | `lane/engine` | `lib/engine/**` (except `graph.ts`), `lib/db.ts`, `lib/env.ts`, `app/api/**`, Supabase DDL | sole committer of `lib/engine/schemas.ts` |
...
**When:** T+0:00 → T+0:20 (SPEC §6 milestone 1). **Where:** A's main checkout,
branch `scaffold/init`. **How:** one Claude Code session with ultracode enabled
runs the workflow script in **Appendix A**. The workflow fans out agents over
disjoint path groups, then loops a verify-fix cycle until the acceptance gate
is green.
...
4. `lib/env.ts` — zod-validates all env vars at boot, **treating every key as
   optional when the corresponding `DEP_*_MODE` is `fixture`** (this is what
   makes keyless UI-lane development work), failing loudly otherwise.
...
**Acceptance gate (the scaffold PR merges only when all four hold):**
...
- Imported to Vercel, deployed, and the production URL renders the hub.
...
- **After YOUR PR squash-merges:**
  `git fetch origin && git reset --hard origin/main`
  Your lane branch restarts from the new `main`. Never *rebase* your own
  squash-merged commits — squash changes patch identity and manufactures
  phantom conflicts. Everything you merged is in `main`; reset loses nothing.
...
writing code. You own lib/engine/* (sole committer of schemas.ts), lib/db.ts,
lib/env.ts, app/api/*, and the Supabase DDL. Never edit outside that list.
...
(3) llm.ts with content-hash cache and the one-repair-pass rule; (4)
/api/extract returning valid Claim[] for a pasted paragraph; (5) /api/formalize
(HF primary, Gemini fallback, model-agnostic prompt); (6) /api/trace
(search dep → fetch dep → readability extract → Gemini judge); (7) /api/stance;
(8) /api/events + db.ts helpers with one retry.
...
# hex/font/radius literals outside tokens.css:
grep -rn --include='*.tsx' --include='*.ts' --include='*.css' \
  -E '#[0-9a-fA-F]{3,8}\b' app components lib | grep -v tokens.css && exit 1
...
|---|---|
| A | `curl -s -X POST :3001/api/extract -d @fixtures/demo/paragraph.json` returns zod-valid `Claim[]`; with network disabled and `DEP_SEARCH_MODE=live`, `dep('search',…)` demonstrably falls live→cached→fixture (log lines show the descent) |
...
| T4 | WORKTREE-PLAN gives Lane E `app/begriffs/page.tsx` while SPEC §5 has Begriffs reading `term_snapshots` | Lane E owns the page and renders through an engine-owned route/helper; if none exists yet, E renders the greyed LACUNA panel — never imports `lib/db.ts` directly |
...
|---|---|---|
| T−1 (before event) | — | §1 credential queue P0 items; repo pushed; Vercel + Supabase accounts live; this doc set on `main` |
| T+0:00–0:20 | Deployed skeleton | Run Appendix A scaffold workflow; import repo to Vercel; set env vars + DEP modes (production = live/live/cached/cached/live per SPEC §7); merge `scaffold/init`; verify deployed URL; create worktrees; open 5 lane sessions with §5 charters |
| T+1:00 | schemas frozen; DDL; dep+fixtures | Run SPEC §3b DDL in Supabase SQL editor; merge Lane A's first PR, then Lane E's; confirm every lane has rebased; deliver Lane D's keys |
...
| T+4:00 | Freeze | Rehearsal #1 with wifi; rehearsal #2 with `DEP_*_MODE=cached` across the board; copy fixes only from here; lift the DATA-CAVEATS judge-facing summary into the README/slide |
...
Run from a Claude Code session at the repo root (A's main checkout) with
ultracode enabled, on a fresh `scaffold/init` branch
(`git switch -c scaffold/init`). The script fans out over disjoint path groups
so agents share the tree safely, then loops verify→fix until green.
...
import "server-only"; schemas.ts follows SPEC §3 verbatim; everything must
work in fixture mode with NO env vars set. Commit your work to the current
branch (scaffold/init) when your group is done.`
...
background var(--stock). Create empty directory structure per SPEC §2. Ensure
pnpm build passes before finishing.`, { label: 'base' })
...
NO .env.local present; the ORCHESTRATION §6 hex grep; node
scripts/seed-fixtures.ts --check. Also verify every app/api route file
exports maxDuration and lib/db.ts imports "server-only". Return green=true
only if ALL pass; otherwise list each failure with file and error.`,
...
gh pr create --fill && gh pr merge --squash    # gate first; A is sole merger
...
gh pr create --title "[lane-x] <feature>" --fill
# after YOUR merge:
git fetch origin && git reset --hard origin/main
